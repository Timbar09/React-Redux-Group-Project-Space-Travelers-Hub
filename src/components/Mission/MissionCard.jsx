...
import {
  BiSolidBadgeCheck as ActiveIcon,
  BiSolidBadge as InactiveIcon,
} from 'react-icons/bi';
  ...
  return (
    ...
    >
      <div className="mb-3 mb-md-0">
        <header
          className={`${styles.missionCardHeader} d-flex align-items-center gap-3 py-2`}
        >
          <h3>{processedName}</h3>
          ...
          >
            <span>{isReserved ? 'Active Member' : 'Not a Member'}</span>
            {isReserved ? <ActiveIcon /> : <InactiveIcon />}
          </span>
        </header>
        ...
        <p className={`${styles.missionCardDescription} ps-4`}>
          ...
          {description.length > 250 && (
            <Link
              ...
              text={
                processedDescription.endsWith('...') ? 'Show More' : 'Show Less'
              }
